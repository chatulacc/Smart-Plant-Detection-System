...
const DEFAULT_THRESHOLDS = {
  soil:  { min: 307, max: 716 }, // 30% and 70% of 1023
  temp:  { min: 18, max: 35 },
  hum:   { min: 30, max: 80 },
  light: { min: 410, max: 920 }, // ~40% and 90% of 1023
};
...
const Settings = ({ onSave }) => {
  ...
  const [thresholds, setThresholds] = useState(() => {
    try {
      ...
      return {
        soil:  { min: 307, max: 716 },
        temp:  { min: 18, max: 35 },
        hum:   { min: 30, max: 80 },
        light: { min: 410, max: 920 },
      };
    }
  });
  ...
  const resetToDefaults = () => {
    if (window.confirm("Reset all alert thresholds to greenhouse standards?")) {
      setThresholds(DEFAULT_THRESHOLDS);
      setToast('Thresholds reset to defaults!');
      ...
      <header className="top-bar">
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button
            ...
            style={{
              width: 'auto', gap: 8, padding: '10px 16px',
              background: 'var(--bg-muted)', color: 'var(--text-primary)', border: '1px solid var(--border)',
